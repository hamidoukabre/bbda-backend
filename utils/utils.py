...
# import numpy as np
# from audiomentations import Compose, AddBackgroundNoise, ApplyImpulseResponse, HighPassFilter, LowPassFilter, TimeStretch
# import librosa
...
# #         [
# #             AddBackgroundNoise(sounds_path=noise_path, min_snr_db=snr, max_snr_db=snr, p=0.8),
...
#     # --- chaîne principale d'augmentation ---
#     augmentation_chain = Compose(
#         [
#             AddBackgroundNoise(sounds_path=noise_path, min_snr_db=snr, max_snr_db=snr, p=0.8),

#             # ✅ réverbération (impulse response)
#             ApplyImpulseResponse(ir_path=ir_path, p=1.),
...
#     # --- filtres passe-bas/passe-haut (ajoutés parfois) ---
#     cut_freq_prob = rng.random()
#     if cut_freq_prob >= 0.60:
#         freq_cuts = Compose(
#             [
#                 LowPassFilter(min_cutoff_freq=2000, max_cutoff_freq=3000,
#                               min_rolloff=12, max_rolloff=36, p=1),
#                 HighPassFilter(max_cutoff_freq=1000, min_cutoff_freq=500,
#                                min_rolloff=12, max_rolloff=36, p=1)
#             ]
#         )
#         augmentation_chain.transforms.append(freq_cuts)
...
#     # --- offset temporel avec probabilité ---
#     if rng.random() > 0.70:
#         offset_signal = time_offset_modulation(signal=signal, time_index=time_index)
#         augmented_signal = augmentation_chain(offset_signal, sample_rate=sr)
#     else:
#         augmented_signal = augmentation_chain(y, sample_rate=sr)
...
def time_offset_modulation(signal: np.ndarray, time_index: int, sr: int = 8000 , max_offset: float = 0.25) -> np.ndarray:
    """Return the signal with a time offset of +- max_offset seconds"""
    offset = random.choice([random.uniform(-max_offset, -0.1),
                            random.uniform(0.1, max_offset)]) if time_index else random.uniform(0.1, max_offset)
    offset_samples = int(offset * sr)
    start = time_index * sr + offset_samples
...
def extract_mel_spectrogram(signal: np.ndarray, sr: int = 8000 , n_fft: int = 1024, hop_length: int = 256, n_mels: int = 256) -> np.ndarray:
    """Extract log-power mel-spectrogram"""
    S = librosa.feature.melspectrogram(y=signal, sr=sr, n_fft=n_fft, hop_length=hop_length, n_mels=n_mels)
    return librosa.power_to_db(S, ref=np.max)


def resize_spectrogram(S: np.ndarray, target_shape=(256, 32)) -> np.ndarray:
    """Resize spectrogram to fixed size (H,W)"""
    H_target, W_target = target_shape
    H, W = S.shape
    # resize height
    if H != H_target:
        S = np.interp(np.linspace(0, H, H_target), np.arange(H), S)
    # resize width
    if W != W_target:
        S = np.interp(np.linspace(0, W, W_target), np.arange(W), S.T).T
    return S
...
def audio_augmentation_chain(signal: np.ndarray, time_index: int, noise_path: str, ir_path: str, rng: np.random.Generator, sr: int = 8000):
    """Apply audio augmentations and return resized spectrograms"""
    ...
    # --- filtres passe-bas/passe-haut (optionnel) ---
    if rng.random() >= 0.60:
        freq_cuts = Compose([
            LowPassFilter(min_cutoff_freq=2000, max_cutoff_freq=3000, min_rolloff=12, max_rolloff=36, p=1),
            HighPassFilter(max_cutoff_freq=1000, min_cutoff_freq=500, min_rolloff=12, max_rolloff=36, p=1)
    ...
    S1 = resize_spectrogram(extract_mel_spectrogram(y))
    S2 = resize_spectrogram(extract_mel_spectrogram(augmented_signal))
...
# ---------------------------
# Fonctions d'index et recherche
# ---------------------------
